...
async function getShopProducts(req, res) {
  ...
  try {
    ...
    var productsSalesCount = await query(
      `select sum(o.quantity) as salescount, productId from products p where p.shopId='${shopId}' inner join orders o on p._id=o.productId group by o.productId;`
    )
...
async function createNewProduct(req, res) {
  ...
  try {
    var shopDetails = await query(
      `select name from shops where _id = '${shopId}'`
    )
  ...
  try {
    ...
    if (body.excludeShopIds && body.excludeShopIds.length) {
      whereConditions.push(
        `shopId NOT IN("${body.excludeShopIds.join('", "')}")`
      )
    }
    ...
    if (whereConditions.length) {
      selectQuery = `select * from products where ${whereConditions.join(
        " AND "
      )}`
    } else {
...
const updateProductById = async (req, res) => {
  ...
  try {
    ...
    let results = await query(
      "UPDATE products SET name= ?, imageUrl=?, description=?, price=?, quantity=? where _id = ?",
      productData
    )
    res.status(200).json({
